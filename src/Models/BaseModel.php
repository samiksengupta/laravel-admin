    ...
    public static function boot()
    {
            ...
            if($model->exists) {
                ...
                foreach(request()->file() as $key => $file) {
                    if(\in_array($key, static::editable())) {
                        ...
                        if(isset($instance->uploadFields[$key])) {
                            $folder = $instance->uploadFields[$key]['folder'];
                            $disk = $instance->uploadFields[$key]['disk'];
                        }
                        ...
                        if(\is_array($file)) {
                            $files = [];
                            foreach($file as $f) {
                                $files[] = $f->store($folder, $disk);
                            }
                            $model->{$key} = implode(',', $files);
                        }
                        else {
                            $model->{$key} = $file->store($folder, $disk);
                        }
                    }
                }
            }
        ...
        static::deleting(function ($model) {
            ...
            foreach($elements as $key => $element) {
                if(($element['type'] ?? false) && \in_array($element['type'], ['file', 'files'])) {
                    $disk = 'public';
                    $instance = with(new static);
                    if(isset($instance->uploadFields[$key])) {
                        $disk = $instance->uploadFields[$key]['disk'];
